...
const create_user = async(request) => {
    ...
    user_data.id = uuidv4(); // Generate a unique ID for the user
    ...
    const { error, value} = schema.user_schema.validate(user_data);

    // If validation fails, error will contain the details

    if(error){
        //envia erro de validação de dados de usuario
        ...
        try {
            const new_user = await user_sql.create_user(value);
            return new_user;
        }catch (error) {
            if(error.code === '23505') {
